...
class MGPhotoHandleManager: NSObject {
    ...
    {
        if objc_getAssociatedObject(asset, MGPhotoHandleManager.MGPhotoAssociate.mg_cacheAssociate!) == nil {
    ...
    public static func asset(hightQuarityFor asset:PHAsset,Size size:CGSize,completion:@escaping ((String) -> Void))
    {
        ...
        if objc_getAssociatedObject(asset, MGPhotoAssociate.mg_assetCacheSizeAssociate!) == nil {

            let associateData = Dictionary<String, Any>()

            objc_setAssociatedObject(asset, MGPhotoAssociate.mg_assetCacheSizeAssociate!, associateData, objc_AssociationPolicy.OBJC_ASSOCIATION_COPY_NONATOMIC)
        }

        /// 获得当前所有的存储
        var assicciateData : Dictionary<String, Any> = objc_getAssociatedObject(asset, MGPhotoAssociate.mg_assetCacheSizeAssociate!) as! Dictionary <String, Any>
