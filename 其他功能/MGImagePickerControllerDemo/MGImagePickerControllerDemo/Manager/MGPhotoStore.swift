...
class MGPhotoStore : NSObject
{
    ...
    public func fetch(groups: @escaping ([PHAssetCollection]) -> Void) -> Void
    {
        ...
        mg_fetchBasePhotosGroup { (group) in
            ...
            MGPhotoHandleManager.resultToArray(group as! PHFetchResult<AnyObject>, complete: { (completeGroup, completeResult) in
                groups(self.mg_handleAssetCollection(completeGroup as! [PHAssetCollection]))
    ...
    public func fetchDefault(groups:@escaping ([PHAssetCollection])-> Void) -> Void
    {
        ...
        mg_fetchBasePhotosGroup { [weak self](result) in
            ...
            strongSelf!.mg_prepare(result, complete: { (defaultGroup) in
                ...
                groups((strongSelf?.mg_handleAssetCollection(defaultGroup))!)
...
extension MGPhotoStore
{
    ...
    {
        MGPhotoHandleManager.resultToArray(PHCollection.fetchTopLevelUserCollections(with: PHFetchOptions()) as! PHFetchResult<AnyObject>) { (topLevelArray, result) in
