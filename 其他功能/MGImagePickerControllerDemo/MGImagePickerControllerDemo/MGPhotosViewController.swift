...
class MGPhotosViewController: UIViewController {
    ...
    var assetResult : PHFetchResult<AnyObject>? {
        willSet{
            ...
            //初始化所有的资源对象
            MGPhotoHandleManager.resultToArray(newValue!) { (assets, resultFetch) in
    ...
    fileprivate lazy var collectionView : UICollectionView = {
        ...
        let collectionView :UICollectionView = UICollectionView(frame: CGRect(x: 0, y: 0, width: self.view.bounds.width, height: self.view.bounds.height - 44),collectionViewLayout:flowLayout)
...
extension MGPhotosViewController : UIViewControllerPreviewingDelegate {
    ...
    func previewingContext(_ previewingContext: UIViewControllerPreviewing, viewControllerForLocation location: CGPoint) -> UIViewController? {
        ...
        //获得索引
        let item = collectionView.indexPath(for: previewingContext.sourceView as! MGPhotosCell)?.item
