...
struct MMMM <T> where T: Codable  {
    static func request<T:Codable>(successed:@escaping (((ListResponse<T>?), _ error: Error?) -> Swift.Void), failure:@escaping ((_ error: Error?)  -> Swift.Void)) {
...
//                        let dict = try JSONSerialization.jsonObject(with: data!, options: JSONSerialization.ReadingOptions.allowFragments)
                        successed(object as? ListResponse<T>, nil)
                    }catch {
                        failure(AFError.ResponseSerializationFailureReason.jsonSerializationFailed(error: error) as? Error)
                    }
                }else { // 其他, 204, 205
...
class ListResponse<T>: URLResponse where T: Codable {
