...
//        backItem.center = CGPoint(x: backItem.center.x, y: (self.topBar.bounds.height - 44) / 2)
        ...
        //响应
        backItem.action(at: .touchUpInside, handle: { [weak self] (_) in
            ...
            DispatchQueue.main.async {
                ...
                if let fileName = fileName, fileName.hasSuffix("GIF") {
                    ...
                    PHImageManager.default().requestImageData(for: asset, options: option, resultHandler: { (data, _, _, _) in
                        if let data = data {
                            let options = MGImageCreatingOptions(
                                scale: UIScreen.main.scale,
                                duration: 0.0,
                                preloadAll: true,
                                onlyFirstFrame: false)
                            cell.mg_imageView.image = UIImage.mg_animatedImage(data: data, options: options)
                            // 自定义
                            // cell.mg_imageView.image.mg_animatedGIFWithData(data:data)
                        } else {
...
//        }
    ...
    fileprivate func mg_index(indexFromAllPhotosToAll index: Int) -> Int {
        ...
        return allAssets.firstIndex(of: currentAsset) ?? 0
    }
