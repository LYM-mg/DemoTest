...
// MARK: UIMenuItem
typealias MenuItemActionClosure = ((UIMenuItem) -> Void)
extension UIMenuItem {
    struct MenuItemAssociateHandle {
        static let _MenuItemHandleValue = UnsafeRawPointer(bitPattern: "_MenuItemHandleValue".hashValue)
    }
    /// 用于替代目标动作回调的方法
    ///
    /// - Parameter handle: 执行的闭包
    class func action(_ title: String, _ handle: @escaping MenuItemActionClosure) -> UIMenuItem {
        //缓存
        objc_setAssociatedObject(self, MenuItemAssociateHandle._MenuItemHandleValue!, handle, objc_AssociationPolicy.OBJC_ASSOCIATION_COPY_NONATOMIC)

        //添加
        return UIMenuItem(title: title, action: #selector(_actionHandle(_:)))
//        self.addTarget(self, action: #selector(UIGestureRecognizer.MG_actionHandle))
    }

    @objc fileprivate func _actionHandle(_ menu: UIMenuItem) {
        //执行
        (objc_getAssociatedObject(self, MenuItemAssociateHandle._MenuItemHandleValue!) as! MenuItemActionClosure)(menu)
