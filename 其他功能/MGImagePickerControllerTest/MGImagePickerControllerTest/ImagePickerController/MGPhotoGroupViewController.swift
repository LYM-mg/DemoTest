...
class MGPhotoGroupViewController: UITableViewController {
    ...
    /// 获取图片之后的闭包
    var mg_completeUsingImage: (([UIImage], [PHAsset]) -> Void)? {
        willSet {
    ...
    /// 获取图片数据之后的闭包
    var mg_completeUsingData: (([Data], [PHAsset]) -> Void)? {
...
extension MGPhotoGroupViewController {
    ...
    func fetchDefaultGroups() {
        DispatchQueue.global().async {
            self.photoStore.fetchDefaultAllGroups { [weak self](allGroups, _) in
