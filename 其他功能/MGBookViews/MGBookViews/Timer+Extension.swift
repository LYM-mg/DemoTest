...
extension Timer {
    ...
    public class func every(_ interval: TimeInterval, block: @escaping (_ timer: Timer) -> Void) -> Timer {
        let timer = Timer.new(every: interval, block)
        timer.start()
        return timer
    }
    ...
    @nonobjc public class func new(every interval: TimeInterval, _ block: @escaping (_ timer: Timer) -> Void) -> Timer {
        var timer: Timer!
